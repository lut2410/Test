...
namespace WebApplication.IntegrationTests
{
    public class UsersControllerTests : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        public async Task GivenValidUser_WhenCreatingNewUser_ThenReturnCreatedUser()
        {
             ...
             * I think we have 3 approach:
             * approach 1: use filter to define the associate action(dont need specific id) => need to change requirements in testcase.
             * approach 2: use middleware to add exact uri you want to the context response, but we will need define dynamic dictionary link to map, quite bulky.
             * approach 3: put header in the action of controller, this is not good.
             */
